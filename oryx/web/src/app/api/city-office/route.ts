...
export async function GET() {
  ...
  const scopedCities = viewer.isPlatformAdmin
    ? null
    : (viewer.operatorProfile?.dashboard_access.includes("city")
        ? (viewer.operatorProfile.city_scope.length ? viewer.operatorProfile.city_scope : [viewer.profile?.city || "Unassigned"])
        : null);

  if (!viewer.isPlatformAdmin && (!viewer.operatorProfile?.is_active || !scopedCities)) {
    return NextResponse.json({ error: "City-office access is not enabled for this user." }, { status: 403 });
  }
  ...
  const cities = Array.from(cityMap.values())
    .filter((row) => !scopedCities || scopedCities.map((city) => city.toLowerCase()).includes(row.city.toLowerCase()))
